...
/* enums */
enum { CurResizeBR, CurResizeBL, CurResizeTR, CurResizeTL, CurNormal, CurResize, CurMove, CurDragFact, CurLast }; /* cursor */
		...
		for (k = start; k <= end; k++)
			for (i = 0; i < LENGTH(keychords); i++)
				if (keychords[i].keys[currentkey].keysym == syms[(k - start) * skip])
					for (j = 0; j < LENGTH(modifiers); j++)
						XGrabKey(dpy, k, keychords[i].keys[currentkey].mod | modifiers[j], root,
